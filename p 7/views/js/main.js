...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
	  var PizzaContain=document.getElementsByClassName("randomPizzaContainer");
	  var hs = determineDx(PizzaContain[0], size);
	  var newwidth = (PizzaContain[0].offsetWidth + hs) + 'px';
    for (var m = 0; m < PizzaContain.length; m++) {
      PizzaContain[m].style.width = newwidth;
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for(c=0; c<47; c++) {
    ...
    elem.basicLeft = (c % b) * x;
    elem.style.top = (Math.floor(c / b) * x) + 'px';
